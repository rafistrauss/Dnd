...
function renderSpellcastingSection(className, level, config) {
  ...
  const abilityName =
    config.spellcastingAbility.charAt(0).toUpperCase() + config.spellcastingAbility.slice(1);
  ...
  if (spellSlotCount > 0) {
    html +=
      '<div class="spell-slots-section"><label>1st Level Spell Slots</label><div class="spell-slots">';
...
function renderFeature(feature, className, level, config) {
  const description =
    typeof feature.description === 'function' ? feature.description(level) : feature.description;
...
function renderUsesFeature(feature, level, config) {
  ...
  if (feature.rollable) {
    const rollFormula = typeof feature.rollFormula === 'function' ? feature.rollFormula(level) : '';
...
function renderPoolFeature(feature, level) {
  const maxPool = typeof feature.maxPool === 'function' ? feature.maxPool(level) : feature.maxPool;
...
function rollFeature(featureName, formula) {
  ...
  showRollResult(featureName, 0, null, formula, 0, 'damage');
}
