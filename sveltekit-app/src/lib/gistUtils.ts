...
export async function saveToGist(character: Character, token: string, description?: string): Promise<string> {
	if (!token) {
...
export async function updateGist(character: Character, token: string, gistId: string): Promise<void> {
	if (!token || !gistId) {
