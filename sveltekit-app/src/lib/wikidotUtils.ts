...
export function parseFeat(html: string, url: string): WikidotFeat {
	...
	const description = text.substring(descStart).trim().substring(0, 500) || 'See source for full description';
...
export function parseAttack(html: string, url: string): WikidotAttack {
	...
	const typeMatch = text.match(/(slashing|piercing|bludgeoning|fire|cold|lightning|acid|poison|radiant|necrotic|force|psychic)/i);
 ...
 */
export function parseFromManualInput(html: string, url: string): WikidotSpell | WikidotFeat | WikidotAttack {
	const contentType = detectContentType(url);
	
	if (contentType === 'unknown') {
		throw new Error('Unable to determine content type from URL. URL must contain spell:, feat:, or weapon:');
	}
 ...
 */
export async function importFromWikidot(url: string): Promise<WikidotSpell | WikidotFeat | WikidotAttack> {
	const contentType = detectContentType(url);
