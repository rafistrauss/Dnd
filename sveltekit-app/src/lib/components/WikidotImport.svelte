<script lang="ts">
  ...
  function extractSpellSummary(description: string): string {
    ...
    const boldMatches = Array.from(description.matchAll(/\*\*(.*?)\*\*/g)).map((m) =>
      m[1].replace(/\.$/, '')
    );
...
>
  ...
  >
              ...
              {#if contentType === 'spell'}
                ...
                <div class="spell-stats">
                  ...
                  <div>
                    ...
                    {spell.components.join(', ').toUpperCase()}{#if spell.material}
                      ({spell.material}){/if}
                  </div>
                  <div>
    ...
    <div class="modal-footer">
      ...
      <button on:click={handleImport} disabled={!selectedItem} class="btn-primary">
