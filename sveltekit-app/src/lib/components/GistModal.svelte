<script lang="ts">
	...
	import { saveToGist, updateGist, loadFromGist, loadGistConfig, saveGistConfig, listUserGists, type GistInfo } from '$lib/gistUtils';
		...
		<div class="modal-body">
			...
			{#if mode === 'save'}
				...
				{:else if gists.length > 0}
					<div class="form-group">
						<div class="gist-list-label">Select a Gist to Update (or leave unselected to create new)</div>
						<div class="gist-list">
							{#each gists as gist}
								<button
									type="button"
								...
								>
									...
									<div class="gist-item-meta">
										...
										<span class="gist-date">Updated: {new Date(gist.updated_at).toLocaleDateString()}</span>
									</div>
								</button>
							{/each}
						</div>
			...
			{:else}
				...
				{:else if token}
					<div class="info-message">No gists found. Enter a Gist ID manually or create a new one in Save mode.</div>
				{/if}
			{/if}
