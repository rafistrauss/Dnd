...
function setupEventListeners() {
  ...
  // Gist buttons
  document
    .getElementById('saveToGistBtn')
    .addEventListener('click', openGistModal.bind(null, 'save'));
  document
    .getElementById('loadFromGistBtn')
    .addEventListener('click', openGistModal.bind(null, 'load'));
  ...
  document
    .getElementById('hitDicePlus')
    .addEventListener('click', adjustHitDiceCount.bind(null, 'add'));
  document
    .getElementById('hitDiceMinus')
    .addEventListener('click', adjustHitDiceCount.bind(null, 'subtract'));
  ...
  ['strength', 'dexterity', 'constitution', 'intelligence', 'wisdom', 'charisma'].forEach(
    (ability) => {
      document
        .getElementById(`${ability.substring(0, 3)}SaveProf`)
...
function updateAllModifiers() {
  ...
  ['strength', 'dexterity', 'constitution', 'intelligence', 'wisdom', 'charisma'].forEach(
    (ability) => {
      ...
      const saveMod =
        modifier + (character.saveProficiencies[ability] ? character.proficiencyBonus : 0);
  ...
  Object.keys(skillAbilities).forEach((skill) => {
    ...
    const skillMod =
      abilityMod + (character.skillProficiencies[skill] ? character.proficiencyBonus : 0);
...
function rollAbilityCheck(ability) {
  ...
  showRollResult(`${abilityName} Check`, modifier, null, '1d20', modifier);
}
...
function rollSavingThrow(ability) {
  ...
  showRollResult(`${abilityName} Save`, totalMod, null, '1d20', totalMod);
}
...
function rollSkillCheck(skill) {
  ...
  showRollResult(`${formattedSkillName} Check`, totalMod, null, '1d20', totalMod);
}
...
function rollAttack(attackIndex) {
  ...
  showRollResult(`${attack.name} Attack`, attackBonus, null, '1d20', attackBonus, 'd20');
}
...
function rollDamage(attackIndex) {
  ...
  const isMelee =
    attack.name.toLowerCase().includes('longsword') ||
...
function rollDivineSmiteFromModal(attackIndex) {
  ...
  if (slotIndex === -1) {
    alert(
      `No available ${level}${level === 1 ? 'st' : level === 2 ? 'nd' : level === 3 ? 'rd' : 'th'} level spell slots!`
    );
    return;
  }
...
function openGistModal(mode) {
  ...
  document.getElementById('saveGistConfirm').style.display =
    mode === 'save' ? 'inline-block' : 'none';
  document.getElementById('loadGistConfirm').style.display =
    mode === 'load' ? 'inline-block' : 'none';
...
async function fetchGists(token) {
  ...
  try {
    ...
    const characterGists = gists.filter(
      (gist) => gist.description && gist.description.includes('D&D Character Sheet')
    );
...
function populateForm() {
  ...
  // Skill proficiencies
  Object.keys(skillAbilities).forEach((skill) => {
    document.getElementById(`${skill}Prof`).checked = character.skillProficiencies[skill] || false;
  });
  ...
  if (character.classFeatures) {
    ...
    if (character.classFeatures.divineSense) {
      [1, 2, 3].forEach((num, idx) => {
        document.getElementById(`divineSense${num}`).checked =
          character.classFeatures.divineSense[idx] || false;
      });
    }
    ...
    if (character.classFeatures.spellSlots) {
      [1, 2, 3, 4].forEach((num, idx) => {
        document.getElementById(`spellSlot1_${num}`).checked =
          character.classFeatures.spellSlots[idx] || false;
      });
    }

    // Channel Divinity
    document.getElementById('channelDivinity').checked =
...
function resetChannelDivinity() {
  ...
  if (config) {
    ...
    if (feature) {
      const maxUses =
        typeof feature.maxUses === 'function' ? feature.maxUses(character.level) : feature.maxUses;
...
function longRest() {
  if (
    !confirm('Take a long rest? This will restore HP, 1/2 of the hit dice, and all class features.')
  ) {
    return;
  }
  ...
  character.hitDice.current = Math.min(
    ...
    character.hitDice.current + restoredDice
  );
  document.getElementById('hitDiceCurrent').value = character.hitDice.current;
...
function rollCustomDice() {
  ...
  showRollResult(`Custom Roll: ${diceNotation}`, 0, null, diceNotation, 0, 'damage');
}
